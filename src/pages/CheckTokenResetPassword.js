...
import { checkToken, setStatus,  } from "~/redux/authSlice"
import { authSelector } from "~/redux/selectors"

const CheckTokenResetPassword = () => {
    
    const status = useSelector(authSelector).status
    const message = useSelector(authSelector).message
    ...
    const tokenURL = token.get("token")

    useEffect(() => {
        document.title = "Xác thực yêu cầu khôi phục mật khẩu"
        if (status === "user") {
            navigate("/")
        }
        else {
            dispatch(checkToken(tokenURL))
        }
    }, [])
    useEffect(() => {
        if (status === "success") {
    ...
    return (
        <div>
            <h3>XÁC THỰC YÊU CẦU KHÔI PHỤC MẬT KHẨU</h3>
            <div className="scene">
            ...
            {status === "OK" ? (
                <>
                    <p className="whoami text-primary fs-2">{message}</p>
                    <p className="whoami text-primary fs-3">Đang chuyển trang</p>
                    {/* <p className="whoami text-primary fs-3 fs-bold">
            Tự động chuyển trang sau: {secondsCoundDown}
          </p> */}
                </>
            ) : (
                <>
                    <p className="whoami text-danger fs-2">{message}</p>
                    <p className="whoami text-primary fs-3">Đang chuyển trang</p>
                    {/* <p className="whoami text-primary fs-3 fs-bold">
            Tự động chuyển trang sau: {secondsCoundDown}
          </p> */}
